...
public:
  ...
  virtual double value(const Point<dim> &p, const unsigned int /*component*/) const override
  {
    ...
    return std::exp(-50.0 * (std::pow(p[0] - 0.5, 2) + std::pow(p[1] - 0.5, 2))) + 0.2 * std::sin(2.0 * numbers::PI * p[0])
  ...
  virtual Tensor<1, dim> gradient(const Point<dim> &p, const unsigned int component) const override
  {
    ...
    grad[0] = -50.0 * (p[0] - 0.5) * std::exp(-50.0 * (std::pow(p[0] - 0.5, 2) + std::pow(p[1] - 0.5, 2))) + numbers::PI * 0.1 * std::cos(2.0 * numbers::PI * p[0]);
...
template <int dim>
void Step6<dim>::create_unstructured_mesh()
{
  const Point<dim> center;
  const double inner_radius = 0.2;
  const double outer_radius = 1.0;

  GridGenerator::hyper_shell(triangulation, center, inner_radius, outer_radius, 6);
